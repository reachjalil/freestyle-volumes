...
if (!dockerAvailable()) {
  ...
  describe('freestyle-volumes on Docker + MinIO', () => {
    ...
    test('create, attach, write, flush, detach, reattach to a fresh sandbox, verify after replacement', async () => {
      ...
      assert.equal(volume.dataPrefix, 'it/v/data');
      ...
      const keys = await stack.listKeys('it/v/data/');
      assert.deepEqual(keys, ['it/v/data/', 'it/v/data/dir/', 'it/v/data/dir/nested.txt', 'it/v/data/emptydir/', 'it/v/data/hello.txt']);
      assert.equal(await stack.readObject('it/v/data/hello.txt'), 'hello from sb1\n');
      ...
      assert.equal(await stack.readObject('it/v/data/hello.txt'), 'replaced by sb2\n');
    ...
    test('read-only attach rejects writes and detaches without flushing', async () => {
      ...
      assert.equal(detached.flushed, true);
      assert.equal((await stack.listKeys('it/v/data/')).includes('it/v/data/should-fail.txt'), false);
    });
    ...
    test('namespaces and subpaths are isolated', async () => {
      ...
      await tenantA.create({ name: 'shared' });
      await tenantB.create({ name: 'shared' });
      ...
      assert.deepEqual(await stack.listKeys('tenant-b/v/shared/'), ['tenant-b/v/shared/', 'tenant-b/v/shared/file.txt']);
      assert.ok((await stack.listKeys('tenant-a/v/shared/')).includes('tenant-a/v/shared/users/bob/secret.txt'));
      ...
      assert.equal(await stack.readObject('tenant-a/v/shared/users/alice/new.txt'), 'from-alice\n');
      await tenantB.delete({ volumeId: 'shared', confirm: 'shared' });
      assert.ok((await stack.listKeys('tenant-a/v/shared/')).length > 0, 'deleting tenant-b/shared leaves tenant-a/shared alone');
    });
    ...
    test('mount failure and timeout inside the sandbox are reported and cleaned up', async () => {
      ...
      assert.equal((await volumes.inspectMount({ sandboxId: sb1, mountPath: '/mnt/broken' })).status, 'absent');
      ...
      await waitFor(async () => stack.exec(sb1, 'ps -o args | grep -c "[r]clone mount"').stdout.trim() === '0', 10000, 'rclone process cleanup');
      assert.equal(stack.exec(sb1, 'grep -c " /mnt/blackhole " /proc/mounts').stdout.trim(), '0');
      assert.equal((await volumes.inspectMount({ sandboxId: sb1, mountPath: '/mnt/blackhole' })).status, 'absent');
    });
    ...
    test('interrupted writes: a crashed mount is stale, reattach resumes the upload, detach flushes it', async () => {
      ...
      assert.equal((await stack.listKeys('it/v/data/')).includes('it/v/data/crash.txt'), false, 'not durable after a crash');
      ...
      assert.equal(detached.flushed, true);
      assert.equal(await stack.readObject('it/v/data/crash.txt'), 'written before crash\n');
    });

    test('busy mounts refuse a normal detach; force detaches without a durability claim', async () => {
      await volumes.attach({ sandboxId: sb1, volumeId: 'data', mountPath: '/mnt/busy' });
      ...
      await waitFor(async () => (await volumes.inspectMount({ sandboxId: sb1, mountPath: '/mnt/busy' })).status === 'absent', 10000, 'forced cleanup');
    });

    test('flush failures are surfaced and leave the mount attached', async () => {
      const slow = new FreestyleVolumes({ storage: stack.storage('it'), sandboxes: dockerSandboxes(), defaults: { writeBackSeconds: 120 } });
      await slow.attach({ sandboxId: sb1, volumeId: 'data', mountPath: '/mnt/flush' });
      assert.equal(stack.exec(sb1, 'head -c 200000 /dev/urandom > /mnt/flush/blob.bin').status, 0);
      stack.pauseStorage();
      try {
        ...
        assert.match(failure.message, /still attached/);
      ...
      assert.equal(detached.flushed, true);
      assert.ok((await stack.listKeys('it/v/data/')).includes('it/v/data/blob.bin'));
    });

    test('delete refuses while attachments are recorded, then destroys only this volume', async () => {
      await volumes.create({ name: 'doomed' });
      ...
      assert.deepEqual(await stack.listKeys('it/v/doomed/'), []);
      assert.ok((await stack.listKeys('it/v/data/')).length > 0, 'other volumes untouched');
