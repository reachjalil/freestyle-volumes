...
| :--- | :--- | :--- |
| [VM lifecycle](https://www.freestyle.sh/docs/vms/lifecycle) | Pause preserves memory, open files and processes; start resumes them. Stop discards memory while retaining disk; only persistent VMs can start again. Deletion is permanent. | Pause is not an upload-completion signal. Stop/crash can leave local cache/state without an uploader; deleting a VM can destroy unflushed data. Our rclone reconnection and pause/resume behavior still need live validation. |
