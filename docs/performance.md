...
`cacheMaxSize` and `cacheMinFreeSpace` use the same explicit-unit syntax and also accept `'off'`. A bare number is refused: rclone reads it as KiB, so `'1024'`, which older documentation described as MiB, meant 1 MiB. `cacheMaxSize` (unbounded by default) is an eviction target, not a strict disk quota, and `cacheMinFreeSpace` (default `'1G'`) makes rclone evict clean cached files when free space on the cache disk drops below it. Neither evicts files that are open or waiting to upload, so writing into a mount faster than the bucket accepts can still fill the disk. Leave headroom for both native workspace data and retained uploads, and seed large datasets straight into the bucket.
