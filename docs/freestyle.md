...
- Give VMs keys limited to their volume with `sandboxCredentials` rather than your process's own key; see [sandbox credentials](api.md#sandbox-credentials) and the README's [example](../README.md#give-each-vm-a-key-for-its-volume-only).
